...
Plans use accessible locator specifications with controlled fallbacks. Locator resolution requires exactly one visible match and never silently selects `.first()`. The recorder executes validated JSON directly. A conservative static converter can import common inline Playwright tests as an alternative starting point and export a validated plan as a Playwright regression test; it reports unsupported abstractions instead of executing project code or guessing.
